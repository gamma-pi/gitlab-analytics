...
def plot_pipeline_status(df: pl.DataFrame, prefix: str = "") -> None:
    """
    Stacked barplot: Running vs Not Running pipelines per subdomain, grouped by domain.
    'running' pipelines are flagged, all others ('success', 'failed', 'skipped') are 'not_running'.
    Adds subtle background shading per domain cluster.
    """
    import matplotlib.pyplot as plt
    import seaborn as sns
    import numpy as np
    ...
    # --- Create run_status eagerly ---
    run_status_series = pl.Series(
        "run_status",
        ["running" if x == "running" else "not_running" for x in df["pipeline_status"].to_list()]
    )
    df = df.with_columns(run_status_series)

    # Aggregate counts per domain, subdomain, run_status
    counts = (
        df.group_by(["domain", "subdomain", "run_status"])
          .agg(pl.count("pipeline_id").alias("count"))
          .sort(["domain", "subdomain"])
    )

    # Fixed colors for clarity
    status_colors = {
        "running": "#247624",     # green
        "not_running": "#8e3333"  # red
    }

    # Unique domains for clustering
    unique_domains = list(dict.fromkeys(counts["domain"].to_list()))

    # Plot setup
    plt.figure(figsize=(16, 8))
    x_positions, labels = [], []
    current_pos = 0
    cluster_spacing, subdomain_spacing, bar_width = 1.0, 0.3, 0.25

    # For background shading
    domain_boundaries = []

    for domain in unique_domains:
        subdomains = counts.filter(pl.col("domain") == domain)["subdomain"].unique().to_list()
        start_pos = current_pos - 0.15  # slightly before first bar
        for i, sub in enumerate(subdomains):
            sub_counts = counts.filter(
                (pl.col("domain") == domain) & (pl.col("subdomain") == sub)
            )

            running_val = sub_counts.filter(pl.col("run_status") == "running")["count"].sum()
            not_running_val = sub_counts.filter(pl.col("run_status") == "not_running")["count"].sum()

            pos = current_pos + i * subdomain_spacing
            x_positions.append(pos)
            labels.append(sub)

            # Plot stacked bars
            plt.bar(pos, running_val, width=bar_width, color=status_colors["running"], label="_nolegend_")
            plt.bar(pos, not_running_val, bottom=running_val, width=bar_width, color=status_colors["not_running"], label="_nolegend_")

            # Add percentage text
            total = running_val + not_running_val
            if total > 0:
                rate = running_val / total
                plt.text(pos, total + 0.5, f"{rate*100:.0f}%", ha="center", va="bottom", fontsize=9)

        end_pos = pos + 0.15  # slightly after last bar
        domain_boundaries.append((start_pos, end_pos))
        current_pos = pos + cluster_spacing

    # Draw subtle background for domain clusters
    for i, (start, end) in enumerate(domain_boundaries):
        plt.axvspan(start, end, color=(0.9, 0.9, 0.9) if i % 2 == 0 else (0.95, 0.95, 0.95), alpha=0.3, zorder=0)

    plt.xticks(x_positions, labels, rotation=45, ha="right", fontsize=9)
    plt.ylabel("Pipeline Counts")
    plt.title("Pipeline staleness Percentage per Subdomain (Grouped by Domain)")

    # Run status legend
    run_status_handles = [
        plt.Rectangle((0, 0), 1, 1, color=status_colors["running"]),
        plt.Rectangle((0, 0), 1, 1, color=status_colors["not_running"])
    ]
    plt.legend(run_status_handles, ["Running", "Not Running"], bbox_to_anchor=(1.05, 1), loc="upper left", title="Pipeline Status")

    plt.tight_layout()
    filename = f"reports/plots/{prefix}pipeline_status_stacked.png"
    plt.savefig(filename, dpi=400)
    print(f"Plot saved as {filename}")
...
def main(csv_path: str) -> None:
    ...
    plot_pipeline_status(df, prefix=prefix)
